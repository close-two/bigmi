...
<article class="page-container">
	<form action="/roles/{{$role->id}}" method="post" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$role->remarks}}" placeholder="" id="" name="remarks">
			</div>
		</div>
		<div class="row cl">
					...
					<dd>
						<dl class="cl permission-list2">
							...
							@foreach($value as $rows)
								<label class="">
									<input type="checkbox" value="{{$rows->id}}" name="nid[]" id="user-Character-1-0-0" @if(in_array($rows->id,$nid)) checked @endif>
									{{$rows->operate}}</label>
							@endforeach
							</dd>
						</dl>
