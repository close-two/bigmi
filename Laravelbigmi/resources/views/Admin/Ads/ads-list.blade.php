...
<div class="page-container">
	<div class="text-c">
		<form action="/adminads" method="get">
		<input type="text" name="keywords" id="" value="{{$request['keywords'] or ''}}" placeholder=" 广告名称" style="width:250px" class="input-text">
		<button name="" id="" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 广告名称</button>
		</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a class="btn btn-primary radius" onclick="picture_add('添加广告','/adminads/create')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加广告</a></span> <span class="r">共有数据：<strong>{{$ads->total()}}</strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr class="text-c">
					...
					<td>{{$row->adsname}}</td>
					<td><img width="210" height="150" title="{{$row->adsname}}" class="picture-thumb" src="{{$row->file}}"></a></td>
					...
					<td class="td-manage">
						...
						@endif
						<a style="text-decoration:none" class="ml-5" onClick="picture_edit('广告编辑','/adminads/{{$row->id}}/edit',{{$row->id}})" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="product_del(this,{{$row->id}})" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
				@endforeach
			</tbody>
...
function picture_stop(obj,id){
	layer.confirm('确认要下架吗？',function(index){
		...
		// alert(id);
		//ajax修改上架状态为下架
		$.get('/adminadsdown',{'id':id},function(data){
			alert(data);
		});
