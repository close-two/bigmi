...
<div class="page-container">
	<div class="text-c">
		<form action="/adminorder" method="get">
			<input type="text" name="keywords" value="{{$request['keywords'] or ''}}" id="" placeholder=" 订单号" style="width:250px" class="input-text">
			...
			@foreach($data as $row)
				<tr class="text-c">
					...
					<td><select>
						...
						<option value="3" @if($row->status==3) selected @endif> 已收货 </option>
						<option value="4" @if($row->status==4) selected @endif> 交易完成 </option>
						<option value="5" @if($row->status==5) selected @endif> 退换货 </option>
					</select></td>
					<td class="f-14 td-manage"><a style="text-decoration:none" class="btn btn-success radius"  href="/adminorder/{{$row->id}}/edit" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="btn btn-danger radius del" onClick="article_del(this,'10001')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a><a href="/adminorder/{{$row->id}}">详情</a></td>
				</tr>
				@endforeach
			</tbody>
		</table>
